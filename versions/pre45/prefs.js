...
const colosseum = GObject.registerClass(
  { GTypeName: "colosseumPrefsWidget" },
  class colosseum extends Gtk.Box {
    _init(params) {
      ...
      for (let i = 0; i < leagues.length; i++) {
        ...
        this["_label" + leagues[i].replaceAll(" ", "")] = builder.get_object(
      ...
      this._settings.bind(
        ...
        builder.get_object("prefs_frequency"),
        "value",
        Gio.SettingsBindFlags.DEFAULT,
      );
      this._settings.bind(
        CONSTANTS.PREF_FOLLOWED_ONLY,
        builder.get_object("prefs_followed_only"),
        "active",
        Gio.SettingsBindFlags.DEFAULT,
      );
      this._settings.bind(
        CONSTANTS.PREF_COMPACT_MODE,
        builder.get_object("prefs_compact_mode"),
        "active",
        Gio.SettingsBindFlags.DEFAULT,
      );
      this._settings.bind(
        CONSTANTS.PREF_POSITION_TOPBAR,
        builder.get_object("prefs_position_topbar"),
        "active",
        Gio.SettingsBindFlags.DEFAULT,
      );
    }
...
const LeagueRow = GObject.registerClass(
  {
    ...
    InternalChildren: ["leagueLabel", "leagueSwitch"],
  },
  class Row extends Gtk.ListBoxRow {
    _init(league, settings, callback) {
      ...
      this._league = league;
      this._leagueLabel.label = CONSTANTS.DISPLAY_NAME[league];
...
const TournamentRow = GObject.registerClass(
  {
    ...
    InternalChildren: ["tournamentLabel", "tournamentSwitch"],
  },
  class Row extends Gtk.ListBoxRow {
